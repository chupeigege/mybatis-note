...
public class Reflector {
  ...
  private void addGetMethods(Class<?> cls) {
    //conflictingGetters 集合的 key 为属性名称， value 为 getter 集合，因为子类可能覆盖父类的getter 方法，所以同一属性名称可能会存在多个getter方法
    Map<String, List<Method>> conflictingGetters = new HashMap<>();
    //步骤1. 获取指定类以及其父类和接口中所有定义的方法
    Method[] methods = getClassMethods(cls);
    //步骤2 ：按照 JavaBean 规范查找 getter 法，并记录到conflictingGetters集合中
    for (Method method : methods) {
      ...
      if ((name.startsWith("get") && name.length() > 3)
        ...
        //将属性名与 getter 方法 对反关系记录到conflictingGetters集合中
  ...
  //为每个方法生成唯一签名，井记录到uniqueMethods集合中
  private void addUniqueMethods(Map<String, Method> uniqueMethods, Method[] methods) {
    for (Method currentMethod : methods) {
      if (!currentMethod.isBridge()) {
        //为每个方法生成唯一签名  例如：Void#setName:name,age
